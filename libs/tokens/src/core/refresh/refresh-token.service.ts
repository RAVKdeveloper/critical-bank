...
import type { GenRefreshTokenDto } from '@lib/tokens/types/gen-tokens.dto'
...
export class RefreshTokenService {
  ...
  public async getRefreshToken<T>(dto: GenRefreshTokenDto<T>): Promise<Base64> {
    ...
    const refreshToken = await this.jwtService.signAsync(encryptData)

    return refreshToken
  }

  public async verifyRefreshToken<T>(refreshToken: JwtToken): Promise<ReturnVerifyRefreshToken<T>> {
    const payload: any = await this.jwtService.verifyAsync(refreshToken) // get token with encrypted data
    const key = this.cfg.env.REFRESH_TOKEN_ENCRYPT_KEY
    const decryptData: string = await this.cryptoService.encrypting.decryptSync(payload, key) // get utf-8 string
