...
module.exports = class Init1727787329388 {
    ...
    async up(queryRunner) {
        ...
        await queryRunner.query(`CREATE TYPE "public"."bank_account_currency_enum" AS ENUM('RUB', 'USD', 'EUR', 'BTC', 'ETH', 'USDT', 'ARC')`);
        await queryRunner.query(`CREATE TYPE "public"."bank_account_payment_system_enum" AS ENUM('MASTERCARD', 'VISA', 'MIR')`);
        await queryRunner.query(`CREATE TYPE "public"."bank_account_affiliation_enum" AS ENUM('PERSONAL', 'CORPORATE')`);
        await queryRunner.query(`CREATE TYPE "public"."bank_account_account_type_enum" AS ENUM('COMMON', 'SAVING', 'DEPOSIT', 'MORTGAGE', 'INVESTING')`);
        ...
        await queryRunner.query(`ALTER TABLE "bank_account" ADD CONSTRAINT "FK_c8d57e8df596573a617476fdff2" FOREIGN KEY ("user_id") REFERENCES "user"("id") ON DELETE CASCADE ON UPDATE NO ACTION`);
    }
